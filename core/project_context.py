...
class ProjectContext:
    ...
    def reload(self) -> bool:
        ...
        Returns:
            True 表示加载成功，False 表示文件不存在或读取失败。
        """
        self._loaded = True
    ...
    def get_system_prompt_section(self) -> str:
        """
        返回要注入 system_prompt 的项目规则文本。
        如果 AGENT.md 未加载（文件不存在/读取失败/内容为空），
        返回空字符串
        """
    ...
    def get_context_str(self) -> Optional[str]:
        """返回可注入 context 的内容（None 表示无需注入）

        Returns:
            若 AGENT.md 已加载返回完整规则文本，否则返回 None。
            此方法为 AgentRuntime.build_context() 的直接数据源。
        """
