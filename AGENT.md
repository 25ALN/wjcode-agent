...
- 只读项目/目录分析（如难点、亮点、架构、结构梳理）只能暴露 `ls` / `grep` / `read_file`，不得同时暴露 `write_file` / `edit_file` / `execute_code` / `web_search`；此类任务不先请求 LLM，而是由 Runtime 创建 `ProjectAnalysisSession`，初始化 Analysis Goal、Checklist、EvidenceStore、CoverageTracker 和 SearchPlanner；Runtime 按 Architecture -> CLI -> Prompt -> Runtime -> Planning -> Memory -> Context -> Permission -> Tool -> Server/Web -> RAG -> Tests -> Deployment 的顺序主动收集证据，Checklist 未满足时拒绝 Final，最终回答必须引用 Evidence
